# STATUS progress log — archive (2026-06-24 … 2026-08-01)

Split out of `docs/STATUS.md` on 2026-08-01: it was 3364 of that file's 3801 lines and
pushed the cold-start read past any usable context budget. Content is **verbatim** —
nothing was summarised or dropped. Newest entry first.

Read this only when you need the history behind a specific decision. Current state lives
in `docs/STATUS.md`; per-item detail in `docs/findings/`.

**Older docs cite "STATUS §0" and "STATUS §6" — both are here now**, §6 as the log below and
§0 as the appendix at the end. `docs/STATUS.md` no longer has those sections.
...
- **2026-08-01 (#125 — there was no cache; the run that failed was mine, and it exported a scene
  ...
  - **The diagnosis it was opened on was wrong, and wrong in the worst direction** — it accused the
    good run and blessed the dead one. **The match that opened this ticket was measured on two
    `scene_rigid.json` files**, and those are exactly the files whose calibration
    `apply_rigid_camera.py` has *overwritten* with #119's single measured camera. Re-measured:
    `carry_rigid` vs `fresh60_rigid` → `max|dH| = 0.000e+00`, bit-identical, because they are
    literally the same numbers written by the same script into two files. It was evidence about the
    script, not about the pipeline. **The RAW calibrations were never equal**: `fresh60` vs
    `carry_off` differ by **88 m on-pitch** (`max|dH| = 7.61e+02`). Comparing post-`_rigid` scenes
    can only ever return "identical" — a comparison that cannot fail is not a check.
  - **The calibrator *is* deterministic, and this run finally proves it properly.** `fresh60` and
    today's re-run of the same clip on the same pod produce **bit-identical confidence across all
    60 frames** (`max|dc| = 0.0`, mean 0.5397, 60/60 solved). Their homographies do differ — 0.76 m
    median, 3.67 m max on-pitch — and that is not noise either: `fresh60` was launched with
    `CAMERA_CARRY=0` and this one took the wiring default `camera_carry=8`. Confidence is scored
    per-frame *before* `carry_on_motion` replaces the track (`calibration.py:586-593`), which is why
    it can be identical while the stored homographies are not. So that 0.76 m is the **first
    measurement of what R2 camera propagation (#94) actually does to this clip** — a feature delta,
  ...
  - **Why PnLCalib solved nothing — answered, and it is the dumbest possible reason: I reconstructed
    a different video.** Not the weights (`$VOL/weights/pnlcalib/{SV_kp,SV_lines}` are intact
    265 MB files, checked), not the repo, not CUDA. `diff` of the two run logs, one line:
    `ingested /runpod/fresh60 … Colombia-1-0-Congo-DR1080p.mp4 @ 29.970fps` versus
    `ingested /runpod/clip.mp4 @ 59.940fps`. `/runpod/clip.mp4` is a different file — 12,133,176 B,
    md5 `2ad8e00e…`, 960 frames, 16.02 s — against Colombia's 5,841,030 B, md5 `a2766d30…`. No
    soccer pitch in it, so PnLCalib found nothing to fit, on every frame, correctly.
  - **`scripts/pod_real_e2e.sh:39` chose it for me:** `CLIP="${PITCH3D_CLIP:-$VOL/clip.mp4}"`, and I
    never set `PITCH3D_CLIP`. **This is the second time.** [2026-07-03](#) is the same trap, same
    file, same stale stock clip, and that entry ends *"Candidate guard for later: wrapper-level
    clip-provenance check … since the default path silently reconstructs whatever
    `/workspace/clip.mp4` happens to be."* The guard was never built, so it happened again — and this
    time it got further, because `apply_rigid_camera.py` made the wreckage look healthy.
  - **The default is gone**, in `pod_real_e2e.sh` and `pod_make_video.sh` both: `PITCH3D_CLIP` is
    now required and the script exits 2 (listing the volume's `*.mp4`) if it is unset, or if the
    named file does not exist. Only the operator knows which footage was meant; a convenience
    default for that argument is a way to reconstruct the wrong match in silence.
  - **The run also said `field calibration confidence mean=0.00`** in its own log, where the healthy
    one says `0.54`, plus `11 subject(s)` against `23` and teleports of 378 m / 633 m / 300 m / 272 m.
    Every number needed was printed. The gate now stops the run at that point instead of printing.
  - **Re-run with the clip named explicitly → `out/pod_0801b`, and it is healthy**: Colombia at
    29.970 fps, **23 subjects**, calibration **60/60 solved, confidence mean 0.54**, worst teleport
    **0.98 m** (against 378 m). That also settles this ticket's second question — *which of 11 vs 23
    is right* — **23**; 11 was the wrong footage, not a tracking regression. Scene pulled local.
...
- **2026-07-31 (#119 — the clip is one camera, and that is measurably a better fit than 480 free
  ...
  - **The 480-parameter solve gets nearly half the camera motion wrong.** Its pixel-motion error by
    ...
    **This, not the `jitter` column, is the jitter result — 0.55 vs 4.08 px is 7.4×, where
    5.31→5.11 is 4%.** `jitter` (`smooth_residual`) assumes cubic smoothness and moves only
    4.99–5.30 across a 1.9× sweep of the focal (below), so it cannot discriminate between cameras;
    it is kept as a guard against a fit that is wildly unsmooth, not as evidence. The honest
    statement of steadiness is the gap-1 pan residual against the 8.55 px the camera really moves.
  ...
  - **The focal is single-valued and seed-independent: f ≈ 4169 px @1920×1080.** Four seeds spanning
    ...
    a gantry 70 m back and 17 m up, which is what a broadcast rig is. **That 0.3 % seed spread is
    not the uncertainty**, only proof of a single basin: the sweep below puts the real
    identifiability at ±10 %.
  - **#117's "focal = 2700" was not noise — the two instruments really do disagree, and the paint
    wins.** Holding the focal and refitting everything else (`--sweep`, 183 free parameters):

    | f held | centre (m) | paint px (p90) | pan px | jitter | f/dist |
    |---|---|---|---|---|---|
    | 2700 | (−10.9, −45.8, 12.6) | 5.22 (27.50) | **0.85** | 4.99 | 55.43 |
    | 3000 | (−9.4, −50.5, 13.8) | 4.67 (16.01) | 0.94 | 5.09 | 56.41 |
    | 3400 | (−7.3, −57.1, 15.2) | 2.64 (8.13) | 1.09 | 5.18 | 57.07 |
    | 3900 | (−4.0, −65.7, 16.6) | 1.53 (5.09) | 1.30 | 5.04 | 57.50 |
    | **4179** | (−1.9, −70.5, 17.4) | 1.38 (**4.49**) | 1.41 | 5.09 | 57.50 |
    | 4600 | (1.1, −78.1, 18.8) | **1.31** (4.97) | 1.59 | 5.27 | 57.27 |
    | 5200 | (5.6, −89.2, 20.8) | 1.78 (7.01) | 1.64 | 5.30 | 56.66 |

    Three things this settles, one of which limits the result:
    - **The paint has a real interior minimum, bracketed on both sides** — p90 runs 8.13 → 5.09 →
      **4.49** → 4.97 → 7.01. It is a bowl, not the plateau it could have been, so the focal is
      genuinely observable and not merely bounded from below. Honest width: within ~0.5 px of the
      p90 floor is **f ≈ 3900–4600, i.e. 4169 ±10 %**. (The free joint fit at f = 4169 reaches 1.28
      (4.01), better than every held point on both statistics — held-at-4179 giving 1.38 (4.49) sets
      the optimiser's own noise at ~0.1 px.)
    - **The pan never votes for a focal, only for "shorter".** Its residual is monotone across the
      whole sweep, 0.85 → 1.64, with no interior minimum at all: at f = 2700 it beats the joint
      fit's 1.44. That is exactly the `K R Rᵀ K⁻¹` flatness #117 identified — with 180 free rotation
      parameters a short focal buys the pan slack — so the pan is load-bearing for *motion* and
      near-useless for *focal*. The paint is the sharp instrument and it rejects 2700 outright
      (p90 27.50). The joint fit spends 0.6 px of pan to save 4 px of paint, and that is the right
      trade.
    - **The limit: focal and distance are not separately observed.** `f/dist` sits at 55.4–57.5
      across the entire sweep while the centre travels from 46 m to 89 m out — the fit holds *image
      scale* fixed and slides the rig along the optical axis. What breaks the tie is the pitch's
      **perspective**, not its scale, and that is a weaker signal: it is why the bowl is ±10 % wide
      rather than ±1 %. So "70 m back" carries ~±10 % with it, and any downstream use that needs
      absolute depth (not reprojection) should be told so.

    Note what this overturns: *"a single plane cannot determine the focal"* (`recalibrate_camera.py`)
    is true of a **single homography** and false of a rigid multi-frame fit, where one plane seen
    from 60 rotations at a shared centre does pin it — to ±10 %. Full log: `/tmp/pan_sweep.log`,
...
# Appendix — STATUS front matter as it stood on 2026-08-01

The 2026-08-01 split rewrote these sections instead of moving them: §0 was a 199-line running
narrative of the v2 tail, and §2/§4 were condensed into `docs/STATUS.md` and `CLAUDE.md`.
They are reproduced **verbatim** here so the split loses nothing.
...
- **NEXT ACTION (2026-08-01):** the current work stream is **geometry honesty**, not v2 photoreal —
  ...
  *bad* camera but **no** camera — nearest realizable pinhole 525 px away at every focal; #119's
  ...
  was the pod script silently reconstructing the wrong video. All three holes are now gated in code
  (`require_solved_calibration`, `PITCH3D_CLIP` required, and poseannot marks a scene drawn over
  footage it was not reconstructed from). **Healthy reference artifact:**
