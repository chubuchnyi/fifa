...
| **R2** #94 ✅ | ~~RAFT-small~~ → **LK corner tracking** + ~~MAD 3σ~~ + RANSAC propagation between PnLCalib anchors | v2 §3.3 | **Shipped 2026-07-30 as `--camera-carry N` (default 8; `0` restores per-frame).** `LucasKanadeMotion` (cv2, CPU) recovers each inter-frame homography; `carry_on_motion` (pure numpy) re-fits every frame from its ±N neighbours by voting in the **physical** domain — homography coefficients are defined only up to scale and must never be averaged. Verified E2E through the real CLI on the target clip: **91.3 % of the exported track's swim removed**, matching the bench's 92 %; the shipped motion backend reproduces the bench's own tracking to **0.0000 px**. **MAD 3σ was deliberately NOT shipped** — "MAD-then-carry" was never scored, and shipping an unmeasured component is exactly what ADR-0012 forbids; MAD alone is dominated (0.102 m vs 0.011 m). Carrying is **not** confidence-weighted — that form is gated on #106. Still-image eval (`run_calib_eval.py`) stays uncarried by construction: unrelated images have no motion to carry. **Do not expect a steadier camera in the render (#107):** `AppController` replaces the solved `CameraTrack` with a synthetic tiled `standard_viewpoints(BROADCAST)` pose before export (measured: `anim_A`'s exported camera-translation spread over 60 frames is `[0,0,0] m`), so all of this reaches the picture only through **subject placement** — the swim reads as players sliding against a static pitch, never as a drifting camera. **The measurement that justified it, 2026-07-29, `scripts/bench_camera_swim.py` (#104):** the calibration really does swim: **median 0.119 m, p95 0.468 m** of frame-to-frame scene slide, on a camera whose true pan is a smooth 9.26 px/frame; 20 % of frames past 0.25 m. Under R7 that time-varying common-mode term is precisely the *visible* error class. Carrying the calibration along the measured inter-frame motion removes **92 % of the median and 93 % of the p95** (±16 window). Two controls say what that is worth. First, the swim metric is **circular** — an anchor displaced **10 m** and carried the same way scores **0.0000 m**, so it measures temporal consistency and is blind to accuracy; it can never justify this alone. Second, an independent check against the painted lines *can* discriminate (a 2 m displaced camera scores **15.3 px** vs our 1.22) — and it says the swim removal is **not free**. Over the full 60-frame span carrying is closer to the paint on only **32 %** of frames, by a median **0.19 px**; swim removal and accuracy trade off **monotonically** across every method tried, with no setting that improves both. What decides it is putting both axes in metres: 1 px at the probe points is **0.0182 m**, so carrying gives up **~0.0035 m** of accuracy to remove **0.108 m** of scene slide — a **31× asymmetric** trade, worth taking but to be reported as a trade, with the operating point exposed as a knob. **No GPU:** RAFT-small exists only to recover the motion the carry rides on, and `goodFeaturesToTrack`+LK recovered it on CPU at 4000/4000 corners / 3790 RANSAC inliers in ~1 min for the clip. | ~~yes~~ **no** |
| **R3** #95 ✅ | ~~Fit line **edges** (IFAB ≤0.12 m)~~ → **use the line detections we already throw away** | v2 §3.3 | **Edge form rejected 2026-07-29 on measurement: the lines are 1–3 px wide** (distance-transform median **2.00 px**, raw FWHM 1–2 px — a single-lobe PSF, not a resolvable band; `scripts/measure_pitch_line_width.py`). **The salvage SHIPS — first brief item to survive measurement.** PnLCalib's line head was discarded at `pnlcalib_backend.py:115`; each detection is now one DLT row `lᵀ·H·x = 0` beside a correspondence's two. Honest headline is **robustness, not precision**: at the target clip's 10 kp/frame, median 0.201 → **0.182 m (−9%)**, p95 0.971 → 0.815 m; at 4 kp p95 **95.8 → 1.26 m**; 3-kp frames go from *unsolvable* to 0.324 m. Repro: `scripts/bench_line_constraints.py` (synthetic; real-clip + B1 numbers still need the pod). It prints an **optimistic** table too (−41% at 10 kp) — the gap is what modelling line error as iid buys you. | no |
| **R4** #96 ✅ | `Provenance{measured,imputed,interpolated}` + `BallMode{on_ground,ballistic,unmeasured}` | brief §3 | **Done 2026-07-29 — and it found a real information loss, not just a type.** Before this, state provenance was encoded as sentinel `subject_frame_conf` values (1.0 measured / 0.3 bridged / 0.2 coasted / 0.15 teleport-interpolated), so a 0.2 was indistinguishable from "measured, detector unsure" — a photoreal renderer could not tell an observed body from an invented one. The **ball** was worse: `on_ground=False` meant *both* "Z from a fitted parabola" *and* "no bracketing contact, Z is a hold" — `ball_lift`'s own comment called those tails "genuinely unknown" while reporting them identically to a real arc. Provenance rides the `(T,…)` arrays in `PoseSequence` (not the brief's parallel per-frame `PlayerState`), is stamped by every gate that fabricates rows, survives JSON + the anim export into the Blender npz (contract `SCHEMA_VERSION` 1→2), and is deliberately **not** folded into `alpha` — fading a coasted player out would be erasing, not marking. Legacy saves migrate `on_ground=False → unmeasured` (the weaker reading); their pose rows read all-`measured` because the fact was never recorded — re-run coherence to restore it. E2E: coherence report `filled=19 / extended=137` on the target clip arrives in the npz as `interpolated=19 / imputed=137`. | no |
| **R5** #97 ❌ | Joint-limit residual: VPoser-norm prior + one-sided knee/elbow hinges | brief §7.3 | **Rejected 2026-07-29 on measurement — the premise is half right and the half that fails is the one that matters.** SMPL-X really has no joint limits, so hyperextension is *representable*; the brief then assumes it is *reachable*. It is not, here: across all **1008** subject-frames of the production variant, **0.0 % of knees and elbows go past straight** (knee min **+11.6°**, median 34.7°; elbow min +23.9°). The pose nets regress into the manifold of the humans they were trained on. The interesting half is that a net is not our only author of poses — R4's provenance labels let the **137 frames `extend_pose_to_span` fabricates** be scored apart, and they come out *safer* than measured ones (knee min **+24.6°**), because the gate **holds** the last articulation instead of extrapolating it. Variant B (SAM 3D Body) is looser — 7.8 % of frames a few degrees past straight, worst **−11.1°** — but that is inside normal genu recurvatum, not a broken rig; nothing anywhere passes the 15° implausibility line. So the residual would buy a VPoser dependency and a term that can only pull a plausible pose *away* from the observations, to enforce what the training distribution already enforces. **Re-opens only with the factor graph**, which is where limits would actually bite (an optimiser fitting pose to observations with no data prior) — not separately. Repro: `scripts/bench_joint_limits.py`, which derives the flexion sign from the body model and self-checks it rather than trusting an axis convention. | no |
| **R6** #98 ✅ | Golden tests: projection round-trip + homography **sign** | brief §9.2.2/9.2.4 | **Done 2026-07-29.** `tests/unit/test_golden_projection_sign.py` (8). The brief asked for a round-trip; a round-trip alone is close to worthless here, and one of the tests exists to *prove* that: mirror the image convention and `image_to_world(world_to_image(x))` still closes to **1e-9** while the same homography puts a known world point **40–66 m** out. So each test anchors to a ground truth built independently of the code (`eval/synthetic`'s GT camera) or to an invariant a mirror breaks — triangle winding, and world +Z ⇒ pixel v *decreases*. Also pins the two in-tree quaternion implementations against each other and against scipy, and puts the first-ever test on the **180°-roll gate** (`-R[1,2] < 0`), which was untested production code. Verified by mutation, not by assertion count: `scripts/mutate_projection_sign.py` breaks the source 5 ways and every one is caught — including MUT-5, the X-only mirror that was eye-validated 2026-07-07 and falsified a day later. | no |
| **R7** #99 ✅ | Our own metric: per-player residual after removing common-mode camera bias; local MPJPE stratified by joint speed (top decile) | brief §1 + §9.4, inverted | **Done 2026-07-29 — and it retires their envelope with a number.** `pitch3d/eval/novel_view.py`. Three error fields pinned to Global MPJPE **0.400 m** (dead centre of the briefs' 0.35–0.45 m band) leave the viewer with **0.002 m**, **0.000 m**, and **0.378 m** respectively — a 190× spread inside one "acceptable" number, so that number cannot rank our work. Reports what survives a *static* camera re-fit, a *per-frame* re-fit (the headline), and `scene_swim_m` between them — because a common-mode error that **moves** slides the whole scene under a locked-off shot and is emphatically not free (the wobble field: 100 % absorbed by a per-frame fit, 0.400 m of swim). Scale is deliberately **not** absorbed — a similarity fit would erase #61's ~3× defect, which is visible against a true-size pitch — and is reported as `predicted_scale` instead. The instrument's own bias is measured, not assumed: on pure per-player scatter it falsely absorbs **70.9 % at 2 subjects, 6.8 % ± 4.0 at our clip's 21** (worst draw 18.8 %), so the residual is a **lower bound**, meaningless below ~8 bodies. Speed stratification confirms the yaw-low-pass trade: with a 120° turn present, Gaussian σ chosen by the mean picks σ=8 while the top decile is best at σ=2 and *degrades* 0.0316→0.0339 beyond it. Repro: `scripts/bench_novel_view_metric.py`. | no |
| **R8** #100 ✅ | Port §9 "rejected and why" → ADR-0012 | v2 §9 | **Done 2026-07-29.** [ADR-0012](adr/0012-rejected-approaches-log.md) — tiered by evidence: Tier 1 rejected on *our* measurement (USAC, the SMPL-X "unify" premise, yaw low-pass, sparse FK sampling, green-key), Tier 2 v2 §9 ported with our own verdict + conditionality, Tier 3 the briefs' prescriptions we declined. Every entry records **what would re-open it**. | no |
...
geometry. Reuse `solve_homography_ransac` for the propagation stage. (Frame-to-frame tracking
is px→px and *is* homoscedastic, so USAC is defensible there specifically; measure before
adopting, and do not let it near the image→world solve.)

**R2 build note (from #104's measurement).** Vote in the *physical* domain, not in homography
coefficients: each neighbour predicts where a probe pixel lands on the pitch, the world points are
combined with a median, and a homography is re-fitted (`fuse()`). Probe **at players' feet**, not over
the whole frame, since error at the horizon is enormous and invisible. Ship the over-smoothing control
with it: a **frozen** camera scores 0.164 m median, ~18× worse than the carried one, which is what
proves the metric is not merely rewarding smoothness.

Measured trade curve (swim median → paint error median), so the operating point is a choice and not
an accident: per-frame **0.119 m → 1.22 px** · MAD-reject k=3 **0.102 → 1.23** · MAD k=1 **0.060 →
1.34** · coefficient average w=17 **0.027 → 1.70** · carried ±8 **0.011 → 1.41** · carried ±16
**0.009**. Two consequences. **`_temporal_smooth` (coefficient averaging, in the tree, default OFF)
is dominated** — carrying beats it on *both* axes at once — so replace it rather than enable it; note
it is not the no-op the first draft of this note called it, it removes 77 % of the swim. And
