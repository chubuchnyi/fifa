...
| ⬜ **M3-9 Kinematic plausibility gate (player physics)** — humans can't teleport or turn on a dime: per-subject **speed / accel / turn-rate limits** (elite ceilings ~10.5 m/s, ~8 m/s²) enforced on the root track. (a) violations = **attention items** (R-6: measured, surfaced — probe `scripts/motion_stats.py`); (b) **limits-aware auto-corrections** through the SAME Correction seam (velocity-clamped / constrained-Kalman-class smoother — NOT a wider moving average); (c) **teleport spikes → identity/stitch review** (they are ID-swaps, smoothing them just smears the error). Measured need (STATUS §6, 2026-07-03): the resolved deliverable carries **1083 accel-violation frames**, subj-1 sp\_max **69.6 m/s**; plumbing probe proved MA(5) smoothing is applied+persisted yet leaves teleport-class errors ~70× over the human limit. M3-8's learned prior plugs the same seam as a stronger (b). | `core/correction` (`coherence`/`engine`) ↔ `core/orchestration` |
