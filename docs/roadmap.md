...
| **R3** #95 ⚠️ | ~~Fit line **edges** (IFAB ≤0.12 m)~~ → **use the line detections we already throw away** | v2 §3.3 | **Edge form rejected 2026-07-29 on measurement: the lines are 1–3 px wide.** Distance-transform median **2.00 px** (p90 2.80; 4.0 px in the nearest band), and raw intensity FWHM **1–2 px** on the nearest line — a single-lobe PSF, not a resolvable band. You cannot fit two independent edges to that. Repro: `scripts/measure_pitch_line_width.py`. **Salvaged, and worth more:** PnLCalib already detects lines and `pnlcalib_backend.py:115` **discards them** on the DLT path (`_lines`) — feed them as point-on-line residuals. Needs no edge resolution. | no |
