...
| **F′ ▲ the real vertical defect: a silent constant** · **DONE** | — | `pose.py` substituted the nominal 0.92 m whenever the backend returned no `pelvis_above_foot` and recorded nothing. Now `PoseSequence.root_z_source`, `GVHMRPoseEstimator.nominal_root_z` and a CLI line. 9 tests, mutation-checked end to end through the estimator. **And then measured, not merely marked:** `make_smplx_pose_height_provider` runs SMPL-X FK at the pose stage when the backend is silent, wired in `wiring.py`. The constant now only survives when the SMPL-X model files are absent. 13 tests, mutation-checked |
| **K ▲ bound how long a lost subject is claimed present** · **BUILT, default off** | hours | R-6 says never blink a lost subject out. Nothing said the claim expires, so `extend_to_span` ran every subject to the full clip: `f236_res896` has **38 subjects × 236 frames, median 37 % measured, worst 2 %** (5 real frames, 231 held), and **47.9 % of its subject-frames sit further than 12 frames from ANY measurement** — the distance the same pass refuses to bridge *between two* of them. `max_extend_frames` (`cec52ae`) clamps it per subject; default `None` keeps the old shape until the eye has judged the A/B. **Measured on the fan clip, 195 s:** subject-frames **3840 → 2254 (−41 %)**, imputed **1955 → 369**, measured **1766 → 1766 — not one measurement lost**, subjects **32 → 32 — nobody deleted**, shortest subject 120 → 27 frames, worst distance from any measurement **105 → 13**, measured share of the scene **46 % → 78 %**. `bash scripts/view_extend_ab.sh`. **Same class as #140 and #142 and the biggest one yet by volume** |
