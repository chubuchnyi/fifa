...
| **F′ ▲ the real vertical defect: a silent constant** · **DONE** | — | `pose.py` substituted the nominal 0.92 m whenever the backend returned no `pelvis_above_foot` and recorded nothing. Now `PoseSequence.root_z_source`, `GVHMRPoseEstimator.nominal_root_z` and a CLI line. 9 tests, mutation-checked end to end through the estimator. **The substitution is still there and is legitimate** — a backend without SMPL-X FK cannot know the offset — it is no longer invisible |
