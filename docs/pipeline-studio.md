...
| **9 Ball 3D** (`per_frame`→traj) | ball 2D + homography | world xyz + `BallMode{on_ground,ballistic,unmeasured}` → 3D arc | `max_speed=35`, `CONTACT_PX=140` (`ball_lift.py`) | set a frame's mode, set apex height | fit parabola to clicked apex | instant |
