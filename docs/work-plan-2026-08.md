...
### W1 — detector input resolution → **wrong conclusion, corrected 2026-08-08 (see W1-bis)**

> ⚠ **This conclusion was wrong.** The numbers below are correct; the conclusion drawn from them
> is not. They measure players found per frame. That is not the thing the resolution affects most.
> Measured on identity instead, 896 reduces identity errors by 31–36 % against 560. See W1-bis
> below. This section is left in place because the measurements are still valid.
...
*Closed 2026-08-07, reopened 2026-08-08.*

### W1-bis — resolution re-measured on identity (2026-08-08)

**What was wrong with W1.** It compared resolutions by counting players found per frame. That
number barely moves (+2 %), so W1 concluded the knob does not matter. But the detector's output
feeds the tracker, and the tracker is where the errors are. Nobody measured that.

**What was run.** `scripts/dump_detections.py` (new) writes detections at a chosen input square,
in the npz format the existing CPU probes read. So the GPU box produces detections once, and every
probe re-runs locally without a GPU. 236 frames, five squares, two clips.

**The control is the new 560 dump, not the old cached one.** They differ: 4461 detections against
4362, same count on 108 of 236 frames. Comparing a new high-resolution run against the old cached
file would have mixed the resolution change with everything else that changed since. All rows
below come from the same script, same box, same day.

**The number being minimised** is mid-pitch identity events: a track that appears or disappears
away from the frame edge. That means the tracker lost or swapped a player mid-play. Same
definition `scripts/mask_cue_ab.py` uses.

| clip | res | players/frame | mid-pitch events | raw tracklets | s/frame |
|---|---|---|---|---|---|
| broadcast 1920×1080 | 560 | 18.23 | 89 | 70 | 0.042 |
| | **896** | 18.63 | **61** | **56** | 0.063 |
| | 1064 | 18.83 | 66 | 62 | 0.063 |
| | 1288 | 19.09 | 73 | 65 | 0.103 |
| | 1512 | 18.33 | 97 | 76 | 0.149 |
| fan 1080×1920 | 560 | 14.54 | 90 | 76 | 0.042 |
| | **896** | 15.11 | **58** | **54** | 0.063 |
| | 1064 | 15.12 | 65 | 60 | 0.063 |
| | 1288 | 15.27 | 75 | 67 | 0.103 |

**Result.** 896 is best on both clips. Identity events drop 31 % on the broadcast clip and 36 % on
the fan clip. Raw tracklets drop from 70 to 56 and from 76 to 54. The cost is 1.5× the detector,
which is the cheapest stage: 0.042 → 0.063 s per frame.

For comparison: the McByte mask cue reduced these events by 14 % and cost 686 s of GPU per pass.

**Going higher makes it worse.** At 1512 the detector finds more boxes than at 560 (18.33 vs
18.23) and produces more identity errors than at 560 (97 vs 89). The extra detections at high
input are duplicates and partial boxes, and each one is another chance for the tracker to attach
an id to the wrong player. So 896 is a minimum, not a ceiling.

**Why the old metric misled.** Players per frame peaks at 1288, which is the second-worst
resolution for identity. The two numbers do not track each other.

**Second probe, same conclusion.** `bench_handover_stitch.py` counts something different — how
many of the merges the П3 rule proposes join two different shirts:
...
Fewer fragments, fewer wrong candidates.

**The best value depends on the clip, so it is not in the code.** It lives in
`config/detector_resolution.yaml`, keyed by clip file name, with the measurements and the
procedure for a new clip written into that file. Resolution order: clip entry →
`--detector-resolution` → the config's `default`. `RFDETRBackend` itself carries no measured
number; constructed bare it uses RF-DETR's own 560. 12 tests cover the lookup, including a
missing and a corrupt config (a bad config returns `None` and the run continues).

Two clips is not a rule. Both happened to land on 896; a third clip may not.
...
|---|---|---|
| **W2 / #137** team labels | yes | 0 unlabelled of 56 tracklets, A 26 / B 30. The fix holds. It never depended on resolution |
| **W3** П3 pre-pose | yes | Conclusion unchanged: still do not relax the 2D stitcher. The numbers improve — 6 accepted merges with 2 kit clashes, against 9 and 5 at 560 |
| **W5** assignment margin | yes | Holds at every resolution. At 896, margin < 0.20 catches 48.7 % of breaking tracks while firing on 8.9 % of rows (lift 5.49×, the best of the sweep) |
| **W13** kit classifier | not applicable | Reads video pixels, not detections. 15 tests green |
| **W9** WorldPose constants | not applicable | Different dataset entirely |
| **W10** BT.709 | not applicable | Property of the video decode, before the detector |
| **W4** handover merge | **no** | Needs a new scene, which needs pose on the GPU. The 24 eye labels are tied to the old scene `out/cue/scene_off.json`; a scene at 896 has different track ids, so the 20/21 score cannot be carried over without a fresh pass by eye. The merge code is unchanged, 9 tests green. What is unknown: how many handovers are left once the detector stops fragmenting tracks |
