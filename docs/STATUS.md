...
| #132 | Player crossings break ByteTrack IDs and fuse per-crop poses (occlusion) | **Measured to a decision 2026-08-05: the ticket splits in two and only the tracking half is real.** *Pose half — not reproduced, dropped.* `scripts/smplestx_occlusion.py --frame 87 --pair 15 85` runs the production SMPLest-X per-crop path on the clip's hardest genuine two-player overlap and it does **not** fuse: 79% and 90% of each mesh lands on its own player, under 12% strays onto the other, and the panel agrees by eye. The overlap is mild anyway (back player fills 0.59 of his box, vs 0.30-0.45 for an unoccluded one), so this clip has no hard occlusion. Depth order does flip, but `estimate_bodies` returns no translation — world position comes from the foot point through the pitch homography, so we never read HMR depth. **PromptHMR is therefore not adopted** (no 2.2 GB ckpt, no 6.5 GB SAM 3, no `interaction` flag); the A/B work stands as the permanent answer that mask prompt, joint pass and cross-person attention all buy nothing here. *Tracking half — reproduced, worse than the ticket says.* Kit colour inside a box is a model-free identity check (`track_continuity.py --kit-scan`): across shot 1, **all 38 player tracks break somewhere and 9 change team**, plus 2 duplicate detections putting two ids on one man. Frame 124, written up as "two players in identical kit", was one of those duplicates — inferred from box geometry without looking; corrected. **Next, in payoff order: (1) reject duplicate detections (same kit, IoU>=0.35, heights within 25% already finds exactly the 2 real ones) via NMS before ByteTrack; (2) refuse to hand an id to a new kit — cannot fix a swap but can mark it, which is R-6; (3) shot-cut detection.** Detail: [`findings`](findings/occlusion-pose-research-2026-08-04.md) |
