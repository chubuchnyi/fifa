...
| #133 | **Human physics: filter motion, solid bodies, no popping, limbs in sync** | opened 2026-08-06 by the user, who noted it had been discussed before and **lost** — hence a findings page, not a chat. Inventory done: **most of it is already built and switched off**. `joint`+`orientation` turned on today (violations 118+11 → 0, 97.8%/98.5% of real angular travel kept, players moved 0.0000 m). Still off: `collision` (solid bodies), `pose_motion_sync` + `contact_probe` + `foot_plant` + `gravity_project` (limbs in sync), `identity` (popping), `jerk_clamp`/`inertia_smooth`/`joint_smooth`. A `full_realism` profile enables nearly all of them and has never been run on the real clip. **Genuinely missing:** these are kinematic clamps, not a simulation — nothing enforces momentum or balance; `pose_motion_sync` moves knees/hips only, no arms; `collision` is one capsule per player, not per limb. Each gate goes on only with the paired measurement (what it fixes / what it costs in real motion), per the yaw-low-pass lesson. Detail: [`findings`](findings/human-physics-requirement-2026-08-06.md) |
