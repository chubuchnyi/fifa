...
| **#140** camera never reaches the scene — **duplicate of #61, fixed by wiring** | 9 of 9 scenes carried `fx = 772 @ 1280×720`. **Not a new defect:** `scripts/apply_rigid_camera.py` has carried the full diagnosis since #119 (*"the closest realizable pinhole is still 525 px away... exactly what 'the ground marks are right but the players are not' looks like from the UI"*). What is missing is that `pod_real_e2e.sh` never applies it — `RIGID_CAMERA` is wired into `pod_make_video.sh`, `pod_physics_ab.sh`, `pod_129_ab.sh` and not into the script every scene here was built with. **Applied to the 60-frame scenes; overlay residual 240.0 px → 8.0 px median**, matched subjects 124 → 1088, common-mode 230 → 5.9 (`scripts/bench_overlay_residual.py`). Confirmed by eye. Remaining residual grows 6.2 → 15.7 px centre-to-edge. ~~Which is where distortion becomes testable.~~ **Re-measured 2026-08-10 and redirected away from optics** (`scripts/bench_principal_point.py`): the **paint** at that same outer radius is **2.97 px** while the overlay reads 15.7, and a camera that draws the pitch to 3 px cannot displace a player by 16 px in the same place. The paint's own growth is **not radial** — binned by axis, `|v−cy|` reads 1.12 / **2.54** / 1.26 / **1.08**, peaking in the middle and falling toward the edge, which no lens does. And the overlay's split is **scatter 9.9 vs common-mode 5.9**, i.e. grounding/association, not a shared camera error. The **principal point is refuted as a cause and is not identifiable here**: `cy` flat over ±900 px, `cx`'s minimum at +600 px (81 % across the frame) with the focal walking alongside — a valley in *(cx, focal)*. Next instrument is the grounding path, not a distortion model. [findings](findings/camera-parameters-dropped-2026-08-10.md) **Next: wire RIGID_CAMERA into pod_real_e2e.sh.** [reply §9](findings/reply-camera-model-gap-2026-08-08.md) |
