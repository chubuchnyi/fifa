...
| #134 | **Pitch-3D scrubber in the UI** — judge poses without rendering | **built 2026-08-06** at `/world`, linked from the app toolbar. Whole pitch in 3D: measured markings + goals + turf from `core/scene/pitch.py` (same geometry the overlay and render use, so the views cannot drift), every subject's 22-joint skeleton at the current frame coloured by team, frame scrubber with play, ids and pelvis trails, free orbit/zoom/pan. Two new endpoints: `/api/world/geometry` (static) and `/api/world/{n}/skeletons` (batched 3D — the per-subject joints endpoint is for the single-player editor and would spend the scrub in HTTP). Kept off `index.html` deliberately: that view is a pelvis-centred single-subject joint editor. Verified: 23 markings / 6 uprights, frame 0 gives 23 subjects (A 10 / B 13) with joint z 0.06-2.13 m — feet on the turf, heads at 2 m — and the client JS passes `node --check`. **Standing request: no full final renders until poses and physics are settled** — use this. |
| #133 | **Human physics: filter motion, solid bodies, no popping, limbs in sync** | opened 2026-08-06 by the user, who noted it had been discussed before and **lost**. Inventory: most of it is **already built and switched off**; `joint`+`orientation` turned on today (violations 118+11 → 0, 97.8%/98.5% of real angular travel kept, players moved 0.0000 m, and the pod A/B **passed the eye**). Still off: `collision`, `pose_motion_sync`, `contact_probe`, `foot_plant`, `gravity_project`, `identity`, `jerk_clamp`/`inertia_smooth`/`joint_smooth`; `full_realism` enables nearly all and has never been run on the clip. **Genuinely missing:** these are kinematic clamps, not a simulation — no momentum or balance; `pose_motion_sync` is knees+hips with no arm term; `collision` is one capsule per player. **Identity half diagnosed 2026-08-06:** 78 mid-pitch births/deaths in shot 1, split by `scripts/identity_failure_kind.py` into **70% association miss** (an unclaimed detection sat a median 6–23 px from the dying track) and **30% detection miss** (out of reach of any tracker change) — so 70% is the ceiling for McByte/masks/re-ID. The cheap knob was then checked and is a **null**: `minimum_matching_threshold` swept both ways (it thresholds 1−IoU, not IoU) — 0.95 halves churn alone but the gain is eaten whole once the #132 kit split runs, because both fix the same failure and each cut is itself a mid-pitch birth+death. Stays at 0.80. **Next: integrate McByte** (our ByteTrack + a temporally-propagated mask cue, MIT) — needs a per-frame mask source and a change to the association step, so it wants a fresh context. Detail: [`findings`](findings/human-physics-requirement-2026-08-06.md) |
