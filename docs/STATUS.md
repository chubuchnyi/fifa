...
| **#140** camera never reaches the scene | **9 of 9 scenes on disk carry `fx = 772.02 @ 1280×720`** — the synthetic fallback — against a 1920×1080 clip whose real focal is ~4200. Includes `out/cue/scene_off.json`, the reference scene for the 24 eye labels. Cause: `camera_from_calibration` refuses (reprojection **471 px**, `realizable=False`) and `controller` silently substitutes `_static_camera`. The refusal is correct (#61); the silent substitution is the defect, and `PlaneCameraFit` is never serialized so a scene carries no record of which it has. **Anything comparing a scene to source pixels — overlay alignment, pitch markings, skeletons on players — was never testing what we thought.** Scene-vs-scene results (identity churn, phantoms, handovers, root speed, the 560/896 ranking) are untouched. [reply](findings/reply-camera-model-gap-2026-08-08.md) |
