...
| #132 | Player crossings break ByteTrack IDs and fuse per-crop poses (occlusion) | **The tracking half is now proven, and the pose half has no test case yet.** Kit colour (Colombia light blue vs Congo DR yellow) sampled inside every box is a model-free identity check — `scripts/track_continuity.py --kit-scan`. Across shot 1 (frames 0-235, 38 plausible player tracks): **all 38 are broken somewhere and 9 change team**, i.e. an id keeps its avatar, kit and motion history while silently moving onto a different human. Two duplicate detections (f34, f124) put two ids on ONE player. **Correction:** frame 124 — written up here on 2026-08-05 as "two players in identical kit, one behind the other" — is nothing of the sort; I had inferred it from box geometry without looking. It is one blue player boxed twice, then id 97 jumping to the yellow player at f128 while the yellow player's own id 85 dies at f127. So the PromptHMR masks-vs-boxes A/B that came back NULL twice (whole frame 0.495 vs 0.477; cropped 0.630 vs 0.625) was never testing occlusion at all. That work still stands as the finding that PromptHMR's cross-person attention is gated by `batch['interaction']` and **upstream leaves it off** — a default multi-person pass is N independent decodes sharing an image encoder; forced on it gives +0.030 on the front player but costs 0.018 vs plain boxes, and all four arms sit at 0.59-0.63. **Real occlusions, re-ranked requiring opposing kits: frame 87, track 85 (YEL) 68% behind track 15 (BLU)** — that is the A2/Stage-B case, and opposing kits mean per-player masks can be verified rather than assumed. SMPLest-X Huge is staged and loads locally (687.2M params; the six SMPL-X assets symlinked from `models/`, three of them absent from the MPI zip and taken from ungated `camenduru/SMPLer-X`); one blocker left, ~15 hardcoded `.cuda()` calls vs our CPU torch, cleared by a two-line shim in the probe rather than a patch to the vendored checkout. Detail: [`findings`](findings/occlusion-pose-research-2026-08-04.md) |
